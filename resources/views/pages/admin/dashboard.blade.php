        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
            ...
            <div class="stat-card group bg-white rounded-2xl p-5 shadow-elevation-sm hover:shadow-elevation-md border border-gray-100 cursor-pointer">
                ...
                <div class="mt-4">
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format($total_pasien) }}</p>
                </div>
            ...
            <div class="stat-card group bg-white rounded-2xl p-5 shadow-elevation-sm hover:shadow-elevation-md border border-gray-100 cursor-pointer">
                ...
                <div class="mt-4">
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format($kunjungan_hari_ini) }}</p>
                </div>
                <div class="mt-3 flex items-center gap-1.5 text-xs text-gray-400">
            ...
            <div class="stat-card group bg-white rounded-2xl p-5 shadow-elevation-sm hover:shadow-elevation-md border border-gray-100 cursor-pointer">
                ...
                <div class="mt-4">
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format($kehamilan_aktif) }}</p>
                </div>
                <div class="mt-3 flex items-center gap-1.5 text-xs text-gray-400">
            ...
            <div class="stat-card group bg-white rounded-2xl p-5 shadow-elevation-sm hover:shadow-elevation-md border border-gray-100 cursor-pointer">
                ...
                <div class="mt-4">
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format(\App\Models\Imunisasi::count()) }}</p>
                </div>
                <div class="mt-3 flex items-center gap-1.5 text-xs text-gray-400">
                    <i class="bi bi-calendar-check"></i>
                    <span>Hari ini: {{ number_format($imunisasi_hari_ini) }} &middot; Persalinan: {{ number_format($persalinan_hari_ini) }}</span>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-4">
                    ...
                    @forelse ($activities as $act)
                        <div class="flex items-start gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                            <div class="p-2 rounded-lg bg-{{ $act->bg }} shrink-0">
                                <i class="bi {{ $act->icon }} text-{{ $act->text }} text-sm"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-900 truncate">{{ $act->title }}</p>
                                <p class="text-xs text-gray-500 mt-0.5 truncate">{{ $act->sub }}</p>
                            </div>
                            <span class="text-xs text-gray-400 shrink-0">{{ $act->time }}</span>
                        </div>
                    @empty
            ...
            <div class="overflow-x-auto">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($recentRecords as $record)
                            <tr class="table-row-hover hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <span class="text-xs font-mono font-semibold text-rose-600 bg-rose-50 px-2.5 py-1 rounded-lg">{{ $record->nomor_rekam_medis }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="text-sm font-semibold text-gray-900">{{ $record->pasien?->nama ?? 'N/A' }}</p>
                                    <p class="text-xs text-gray-400">{{ $record->pasien?->nik ?? '' }}</p>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if ($record->pasien?->jenis_kelamin === 'L')
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-blue-50 text-blue-700 text-xs font-semibold">Laki</span>
                                    @elseif($record->pasien?->jenis_kelamin === 'P')
                                ...
                                <td class="px-6 py-4">
                                    @if ($record->diagnosa)
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-blue-50 text-blue-700 text-xs font-semibold">{{ Str::limit($record->diagnosa, 50) }}</span>
                                    @else
                                        <span class="text-xs text-gray-400">-</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-sm text-gray-600">{{ $record->tanggal_pemeriksaan->format('d/m/Y') }}</span>
                                        <span class="text-xs text-gray-400">{{ $record->tanggal_pemeriksaan->format('H:i') }} WIB</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-600">{{ $record->user?->name ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @php $status = $record->tanggal_pemeriksaan->isToday() ? 'Baru' : ($record->tanggal_pemeriksaan->isYesterday() ? 'Kemarin' : 'Lengkap'); @endphp
                                    @if($record->tanggal_pemeriksaan->isToday())
                ...
                @if ($recentRecords->hasPages())
                    <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50/50">
                        <p class="text-sm text-gray-500">Menampilkan {{ $recentRecords->firstItem() ?? 0 }} – {{ $recentRecords->lastItem() }} dari {{ $recentRecords->total() }}</p>
                        <div class="flex gap-1">{{ $recentRecords->links() }}</div>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div>
        ...
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Selamat datang, {{ auth()->user()->name }}! Silakan pilih menu untuk mulai bekerja.</p>
                </div>
        ...
        <div class="bg-white border border-gray-100 shadow-elevation-sm rounded-3xl p-6 sm:p-8 mb-8">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    <h3 class="font-semibold text-xl text-gray-900">Halo, {{ explode(' ', auth()->user()->name)[0] ?? 'Pegawai' }}!</h3>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                <!-- Imunisasi -->
                <a href="{{ route('immunization.index') }}"
