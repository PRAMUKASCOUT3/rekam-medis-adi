...
<div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            ...
            <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ number_format($stats['total_stok']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Stok Aman (&ge;10)</p>
            <p class="text-2xl font-bold text-emerald-600 mt-0.5">{{ number_format($stats['stok_aman']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Stok Menipis (&lt;10)</p>
            <p class="text-2xl font-bold text-amber-600 mt-0.5">{{ number_format($stats['stok_menipis']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Stok Habis (0)</p>
            <p class="text-2xl font-bold text-red-600 mt-0.5">{{ number_format($stats['stok_habis']) }}</p>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($obats as $i => $o)
                        ...
                        <tr class="table-row-hover hover:bg-gray-50/60 transition-colors">
                            <td class="px-6 py-4 text-center text-sm text-gray-500">{{ $obats->firstItem() + $i }}</td>
                            <td class="px-6 py-4">
                                <span
                                    class="text-xs font-mono font-semibold text-gray-600 bg-gray-100 px-2 py-0.5 rounded">{{ $o->kode }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-sm font-semibold text-gray-900 leading-tight">{{ $o->nama }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <span
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-sm text-gray-600">{{ $o->satuan }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-md {{ $stockClass }} text-xs font-bold">
                                    {{ number_format($s) }}
                                </span>
                                <p class="text-xs text-gray-400 mt-0.5">{{ $stockLabel }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-600">{{ $o->created_at }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
        ...
        @if ($obats->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50/50">
                <p class="text-sm text-gray-500">
                    Menampilkan
                    <span class="font-semibold text-gray-700">{{ $obats->firstItem() ?? 0 }}</span>
                    –
                    <span class="font-semibold text-gray-700">{{ $obats->lastItem() }}</span>
                    dari
                    <span class="font-semibold text-gray-700">{{ $obats->total() }}</span>
                    obat
                </p>
