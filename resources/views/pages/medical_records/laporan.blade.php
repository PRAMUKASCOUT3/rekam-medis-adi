...
<div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            ...
            <p class="text-xl font-bold text-rose-600 mt-0.5">{{ number_format($stats['total_periode']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Laki-laki</p>
            <p class="text-xl font-bold text-blue-600 mt-0.5">{{ number_format($stats['laki']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Perempuan</p>
            <p class="text-xl font-bold text-pink-600 mt-0.5">{{ number_format($stats['perempuan']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Dengan Obat</p>
            <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ number_format($stats['dengan_obat']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Tanpa Obat</p>
            <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ number_format($stats['tanpa_obat']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Ada Diagnosa</p>
            <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ number_format($stats['dengan_diagnosa']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            <p class="text-xs text-gray-500 font-medium">Hari Ini</p>
            <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ number_format($stats['hari_ini']) }}</p>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($records as $i => $r)
                        <tr class="table-row-hover hover:bg-gray-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-xs font-mono font-semibold text-rose-600 bg-rose-50 px-2.5 py-1 rounded-lg">{{ $r->nomor_rekam_medis }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <p class="text-sm font-semibold text-gray-900 leading-tight">{{ $r->pasien?->nama ?? 'N/A' }}</p>
                                    <p class="text-xs text-gray-400">{{ $r->pasien?->nik ?? '' }}</p>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                @if ($r->diagnosa)
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-blue-50 text-blue-700 text-xs font-semibold">{{ Str::limit($r->diagnosa, 55) }}</span>
                                @else
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm text-gray-600">{{ $r->tekanan_darah ?: '-' }}</span>
                                    <span class="text-xs text-gray-400">{{ $r->suhu_tubuh ?: '-' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm text-gray-600">{{ $r->tanggal_pemeriksaan->format('d/m/Y') }}</span>
                                    <span class="text-xs text-gray-400">{{ $r->tanggal_pemeriksaan->format('H:i') }} WIB</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
