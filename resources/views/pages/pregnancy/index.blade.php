...
<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <div class="bg-white rounded-2xl p-5 shadow-elevation-sm border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-900 mt-1">{{ $totalRecords }}</p>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-elevation-sm border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-900 mt-1">{{ $todayRecords }}</p>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-elevation-sm border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-900 mt-1">{{ $monthRecords }}</p>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-elevation-sm border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-900 mt-1">{{ $thirdTrimesterRecords }}</p>
        </div>
    </div>
                        ...
                        <th class="text-left px-6 py-3.5 cursor-pointer select-none" wire:click="sortBy('tanggal')">
                            <div class="flex items-center gap-1.5">
                                ...
                                @if ($sortColumn === 'tanggal')
                                    <i class="bi bi-caret-{{ $sortDirection === 'asc' ? 'up-fill' : 'down-fill' }} text-rose-600"></i>
                                @else
                        ...
                        <tr class="table-row-hover hover:bg-gray-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-1 text-xs">
                                    <span class="px-2 py-1 rounded-lg bg-blue-50 text-blue-700 font-semibold">G{{ $record->gravida }}</span>
                                    <span class="px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 font-semibold">P{{ $record->partus }}</span>
                                    <span class="px-2 py-1 rounded-lg bg-rose-50 text-rose-700 font-semibold">A{{ $record->abortus }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center">
                                    <span class="inline-flex items-center gap-1 px-3 py-1 rounded-lg {{ $this->calcWeeks($record->hpht) !== null && $this->calcWeeks($record->hpht) >= 28 ? 'bg-purple-50 text-purple-700' : 'bg-indigo-50 text-indigo-700' }} text-sm font-semibold">
                                        <i class="bi bi-clock-history text-xs"></i>
                                        {{ $this->calcWeeks($record->hpht) !== null ? $this->calcWeeks($record->hpht) . ' minggu' : '-' }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col gap-1">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="text-xs text-gray-700">{{ $record->hpht ? $record->hpht->format('d M Y') : '-' }}</span>
                                    </div>
                                    <div class="flex items-center gap-1.5">
                                        <span class="text-xs font-medium text-gray-400">TP:</span>
                                        <span class="text-xs text-gray-700 font-semibold">{{ $record->tp ? $record->tp->format('d M Y') : ($record->hpht ? $record->hpht->addWeeks(40)->format('d M Y') : '-') }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
            ...
            <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-bold text-gray-900">
                        {{ $editMode ? 'Edit Data Kehamilan' : 'Tambah Data Kehamilan' }}
                    </h2>
                    <p class="text-sm text-gray-500 mt-0.5">
                        {{ $editMode ? 'Perbarui data pemeriksaan ibu hamil' : 'Input data pemeriksaan ibu hamil baru' }}
                    </p>
                </div>
                    ...
                    <div class="flex items-end">
                        <div class="w-full bg-indigo-50 rounded-lg px-3 py-2 border border-indigo-100">
                            ...
                            <p class="text-sm font-bold text-indigo-700 mt-0.5">
                                {{ $this->gestationalAgeWeeks() !== null ? $this->gestationalAgeWeeks() . ' minggu' : '-' }}
                            </p>
                        </div>
                    </div>
                </div>
