    ...
    <aside id="sidebar"
        ...
        <div class="flex flex-col h-full">
            ...
            <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1">
                ...
                @foreach ($menuItems as [$page, $label, $icon])
                    <a href="{{ route($page . '.index') }}" data-page="{{ $page }}"
                        class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-xl text-blue-100 hover:text-white hover:bg-white/10 transition-all duration-200 group {{ request()->routeIs($page . '.*') ? 'bg-white/10 text-white border-l-4 border-blue-400' : 'border-l-4 border-transparent' }}">
                ...
                <div class="mt-1 space-y-1" id="reportDropdown">
                    ...
                    <div id="reportDropdownMenu" class="hidden overflow-hidden space-y-1 pl-4">
                        @foreach ($reportItems as [$page, $label, $icon])
    ...
    <div id="contentWrapper" class="lg:ml-64 min-h-screen transition-all duration-300">
        ...
        <footer class="mt-12 py-6 border-t border-gray-200 bg-white">
            <div class="px-6 text-center text-sm text-gray-500">
                <p>&copy; {{ date('Y') }} Rekam Medis Bidan. Crafted with <i
