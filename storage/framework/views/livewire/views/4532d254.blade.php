...
<div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            ...
            <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ number_format($stats['trim_1']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
        ...
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
            ...
            <p class="text-2xl font-bold text-rose-500 mt-0.5">{{ number_format($stats['trim_3']) }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-elevation-sm border border-gray-100">
                        ...
                        <tr class="table-row-hover hover:bg-gray-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    ...
                                    <p class="text-xs text-gray-400">
                                        @if ($r->umur_istri)
                                            {{ $r->umur_istri }} tahun
                                            @if ($r->alamat) &middot; {{ Str::limit($r->alamat, 30) }} @endif
                                        @elseif ($r->alamat)
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-1.5 text-xs">
                                    <span class="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 font-bold">G{{ $r->gravida }}</span>
                                    <span class="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 font-bold">P{{ $r->partus }}</span>
                                    <span class="px-2.5 py-1 rounded-lg bg-rose-50 text-rose-700 font-bold">A{{ $r->abortus }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @php
                                ...
                                @endphp
                                @if ($weeks !== null)
                                    <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg {{ $weeks >= 28 ? 'bg-purple-50 text-purple-700' : 'bg-indigo-50 text-indigo-700' }} text-sm font-semibold">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col text-xs">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="text-gray-700">{{ $r->hpht ? $r->hpht->format('d/m/Y') : '-' }}</span>
                                    </div>
                                    <div class="flex items-center gap-1.5">
                                        <span class="text-gray-400 font-medium">TP:</span>
                                        <span class="font-semibold text-gray-700">{{ $r->tp ? $r->tp->format('d/m/Y') : '-' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
